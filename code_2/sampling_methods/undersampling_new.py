    ...
    for matrix_id in train_matrix_ids:
        ...
        for i in range(config['N']):
            if config['ratio'] * len(smaller_df) <= len(larger_df):
                ...
                '''
                # We discussed and decided to do nothing.
                ...
                '''
                merged_df = pd.concat([smaller_df, larger_df])
                print(len(smaller_df), len(larger_df))
        
            us_dest_data_dir = os.path.join(config['dest_data_dir'], 'undersampled_ratio_'+str(config['ratio'])+'_'+str(i), 'matrices')
    ...
    for matrix_id in train_matrix_ids:
        ...
        for i in range(config['N']):
            if(len(value_1_label_0_df) <  len(value_1_label_1_df)):
                smaller_df = value_1_label_0_df
                larger_df = value_1_label_1_df
                if config['ratio'] * len(smaller_df) <= len(larger_df):
                    sampled_larger_df = larger_df.sample(n=config['ratio']*len(smaller_df))
                    merged_df = pd.concat([smaller_df, sampled_larger_df])
                    print(len(smaller_df), len(sampled_larger_df))
                ...
                smaller_df = value_1_label_1_df
                larger_df = value_1_label_0_df
            ...
            if(len(value_2_label_0_df) < len(value_2_label_1_df)):
                smaller_df = value_2_label_0_df
                larger_df = value_2_label_1_df
                ...
                merged_value2_df = merged_df
            else:
                smaller_df = value_2_label_1_df
                larger_df = value_2_label_0_df
            ...
            us_dest_data_dir = os.path.join(config['dest_data_dir'], 'undersampled_frac_ratio_'+str(config['ratio'])+'_'+str(i), 'matrices')
            if not os.path.exists(us_dest_data_dir):
                os.makedirs(us_dest_data_dir)
...
def copy_test_yaml(conn, config):
    ...
    for matrix_id in test_matrix_ids:
        ...
        for i in range(config['N']):
            if config['mode']==1:
                dest_file_name = os.path.join(config['dest_data_dir'], 'undersampled_ratio_'+str(config['ratio'])+'_'+str(i), 'matrices')
                
            elif config['mode']==2:
                dest_file_name = os.path.join(config['dest_data_dir'], 'undersampled_frac_ratio_'+str(config['ratio'])+'_'+str(i), 'matrices')
