...
class RecallAdjuster(object):
    def __init__(
        ...
        elif(self.params['dataset'] == 'donors'):
            print("Running Donors Pre-Recall Adjustment")
            #sql = Template(open('donors_recall_adjustment_pre.sql.tmpl', 'r').
            sql = Template(open('SAVE_donors_recall_adjustment.pre.sql.tmpl', 'r').read()).render(**self.params)
            print(sql)
            print("Completed Pre-Recall")
        else:
