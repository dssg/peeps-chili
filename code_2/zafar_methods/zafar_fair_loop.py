...
def get_train_test_matrix_pairs(engine, experiment_hash, model_group_id):
    ...
    (
        ...
        train_results_sep18.prediction_metadata
    ),
    test_matrices as 
    (
        select model_id, matrix_uuid from 
        test_results_sep18.prediction_metadata
    ),
    matrix_info as 
    (
...
def save_output_info(out_path, test_matrix_uuid, train_score, test_score,cov_all_train, cov_all_test, s_attr_to_fp_fn_train, s_attr_to_fp_fn_test, recall_nonwhite, recall_white):
    ...
    with open(os.path.join(out_path, "INFO_"+str(test_matrix_uuid)), 'w') as fp:
        json.dump({
            ...
            'recall_nonwhite':recall_nonwhite,
            'recall_white':recall_white
...
def run_for_all_train_test_matrices(file_path, train_config,train_test_matrices, entity_to_attrib, out_path, test_results_table, test_results_schema):
    ...
    for i in range(len(train_test_matrices)):
        ...
        if (not os.path.exists(out_file)):
            ...
            save_output_info(out_path, test_matrix_uuid, train_score, test_score,   cov_all_train, cov_all_test, s_attr_to_fp_fn_train, s_attr_to_fp_fn_test, recall_nonwhite, recall_white)
...
train_config, test_results_table, test_results_schema, out_path):
    ...
    """
    df_model_id = pd.read_sql(model_id_query, conn)
    model_id = df_model_id['max'].values[0]
    model_id = model_id + 1
    ...
    df_model_group_insert.to_sql('zafar_model_groups', conn, schema='model_metadata', index=False, if_exists='append')
