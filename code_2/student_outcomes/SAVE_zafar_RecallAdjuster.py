...
class RecallAdjuster(object):
    def __init__(
        ...
        self.params['start_model_id'] = start_model_id
        self.params['end_model_id'] = end_model_id
        ...
        if(self.params['dataset'] == 'elsal'):
            ...
            print("Completed Pre-Recall")
        elif(self.params['dataset'] == 'donors_pre'):
            print("Running Donors Pre-Recall Adjustment")
            sql = Template(open('donors_recall_adjustment_pre.sql.tmpl', 'r').read()).render(**self.params)
            print("Completed Pre-Recall")
        else:
            sql = Template(open('SAVE_zafar_recall_adjustment_pre.sql.tmpl', 'r').read()).render(**self.params)
        ...
        print("Running Recall Adjustment")
        sql = Template(open('SAVE_zafar_recall_adjustment.sql.tmpl', 'r').read()).render(**self.params)
        self.engine.execute(sql)
        self.engine.execute("COMMIT")
