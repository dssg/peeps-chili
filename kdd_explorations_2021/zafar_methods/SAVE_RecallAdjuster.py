...
class RecallAdjuster(object):
    def __init__(
        ...
        if(self.params['dataset'] == 'elsal'):
            ...
            sql = Template(open('SAVE_recall_adjustment_pre.sql.tmpl', 'r').read()).render(**self.params)
        ...
        sql = Template(open('SAVE_recall_adjustment.sql.tmpl', 'r').read()).render(**self.params)
        self.engine.execute(sql)
        self.engine.execute("COMMIT")
